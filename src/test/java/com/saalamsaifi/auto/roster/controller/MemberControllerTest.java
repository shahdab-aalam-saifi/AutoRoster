...
public class MemberControllerTest {
	...
	private static final String GROUP_WITHOUT_MEMBER_JSON = "{\"name\":\"Team Name\",\"maxWfrlAllowed\":1,\"groups\":[{\"name\":\"Group Name\",\"maxWfrlAllowed\":1}]}";
	private static final String GROUP_WITH_MEMBER_JSON = "{\"name\":\"Team Name\",\"maxWfrlAllowed\":1,\"groups\":[{\"name\":\"Group Name\",\"maxWfrlAllowed\":1, \"members\": [{\"name\": \"Member Name\"}]}]}";
	...
	public void addNewMember_WhenMemberIsNotExist_ResponseCode200() {
		Collection collection = teamController.add((@Valid Team) stringToJsonObject(GROUP_WITHOUT_MEMBER_JSON, Team.class))
				.getBody();
		GROUP_ID = collection.getGroups().get(0).getId();
		
		ResponseEntity<Collection> response = controller.add(GROUP_ID, Member.builder().name("Member Name").build());
		assertEquals(HttpStatus.OK.value(), response.getStatusCodeValue());
	}

	@Test
	public void addNewMember_WhenMemberIsExist_ResponseCode200() {
		Collection collection = teamController.add((@Valid Team) stringToJsonObject(GROUP_WITH_MEMBER_JSON, Team.class))
				.getBody();
		GROUP_ID = collection.getGroups().get(0).getId();
	...
	public void addNewMember_WhenGroupIsNotExist_ResponseCode400() {
		...
		ResponseEntity<Collection> response = controller.add("G00000", Member.builder().name("Member Name").build());
	...
	public void updateMember_WhenMemberIsNotExist_ResponseCode400() {
		...
		ResponseEntity<Collection> response = controller.update("M0000", Member.builder().name("Member Name").build());
		assertEquals(HttpStatus.BAD_REQUEST.value(), response.getStatusCodeValue());
	}
	...
	public void updateMember_WhenMemberIsExist_ResponseCode200() {
		...
		ResponseEntity<Collection> response = controller.update(MEMBER_ID, Member.builder().name("Member Name").build());
		assertEquals(HttpStatus.OK.value(), response.getStatusCodeValue());
	}
